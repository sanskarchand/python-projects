    ...
    def mangdl(self):
        try:
            ...
            if not os.path.isfile(x + '.html'):
                urllib.urlretrieve('http://www.mangareader.net'\
...
def chapDL(index, listChap, mangaName):
    ...
    namelist = []
    PAGES = 45 # Checks for PAGES pages
    for i in range(1, PAGES):
        ...
        if '-' in urlH and is_int(urlH[urlH.index('-')+1]):
            ...
            print "OURLINK: ", copy
            urllib.urlretrieve("http://www.mangareader.net"+copy, fileS)
        else:
            urllib.urlretrieve("http://www.mangareader.net"+urlH+'/'+str(i), fileS)
        ...
        for each in chitra:
            ...
            urllib.urlretrieve(link, mangaName+'/'+listChap[index]+'/'+name)
    '''
    newJ = 1
...
if not os.path.isfile("mnrdl_list.html"):
    urllib.urlretrieve("http://www.mangareader.net/alphabetical",\
                       "mnrdl_list.html")
